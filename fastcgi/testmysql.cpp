...
string get_mysql_data()
{
    ...
    try {
            ...
            con = driver->connect("tcp://127.0.0.1:3306", "root", "jayant");
            /* Connect to the MySQL test database */
            con->setSchema("test");
            stmt = con->createStatement();
            res = stmt->executeQuery("select * from books");

            result.append("<table><tr><td>name</td><td>author</td></tr>");
            ...
            while (res->next()) {
                ...
                result.append("</td><td>");
                result.append(res->getString("author"));
                result.append("</td></tr>");
            }
            result.append("</table>");
            delete res;
...
int main(void) {
    ...
    while (FCGX_Accept_r(&request) == 0) {
        ...
        string mysqldata = get_mysql_data();
